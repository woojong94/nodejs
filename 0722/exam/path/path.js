...
/*
F:\우종\nodejs\0722\exam\path

F:\우종\nodejs\0722\exam\,,\test_folder\test.js
*/

// path.parse - root, dir, name, ext, base
const parse = path.parse(fileName);
console.log('parse', path.parse(fileName));	// 파일 경로 분해
console.log('format', path.format(parse));	// 분해된 경로를 문자열로 결합하여 반환


// path.normalize --> 비정상 경로를 정상 경로로 변경
const wrongpath = 'F://path\\\\test.js';
